...
# checking index of corpus are the same ad docid
# for i, k in enumerate(corpus.keys()):
#     if i != int(k):
#         raise ValueError(
#             "l'index utiliser pour BM25 n'est pas le même que celui du dataset")
# else:
#     print("Index OK")
#     bm25 = BM25()
#     bm25.fit(corpus)
...
with gzip.open(train_filepath, 'rt') as fIn:
    ...
    for line in tqdm(fIn, unit_scale=True):
        ...
        if (cnt % (pos_neg_ration+1)) == 0:
            ...
            label = 0
        # print("i'am here")
        # print(str(corpus_id), str(query))
        bm25_score_per_doc = index_reader.compute_query_document_score(str(corpus_id), str(query))


        # bm25_score_per_doc = bm25.predict_top_n(query, n=100)
        bm25_normalized = min_max_global(bm25_score_per_doc, 0, 50)
        bm25_score = int(bm25_normalized * 100)
        # train_samples.append(InputExample(texts=[query, passage], label=label))
        train_samples.append(InputExample(texts=[f'{query} [SEP] {bm25_score}', passage], label=label))
